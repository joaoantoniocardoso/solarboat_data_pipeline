...
def convert_json_db_to_candump_log_row(
    element: dict, debug: bool = False, timezone_str: str = "America/Sao_Paulo"
) -> str:
    """
    ...
    date = pd.to_datetime(element["date"])
    if date.tzinfo is None:
        date = date.tz_localize(timezone_str)
    else:
        date = date.tz_convert(timezone_str)

    timestamp = format(date.timestamp(), "10.6f")
...
def convert_json_db_to_candump_log_file(
    input_filename: str, output_filename: str, debug=False
):
    os.makedirs(os.path.dirname(output_filename) or ".", exist_ok=True)
