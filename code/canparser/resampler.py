...
def fix_data_outliers_limits(df: pd.DataFrame, upper: float, lower: float) -> pd.DataFrame:
...
def process_chunk(
    ...
    resample_period_in_seconds = pd.to_timedelta(
        dataset_info['resample_period']
    ).to_numpy().astype(float) * 1e-9
    ...
    return (
        df
        .resample(dataset_info['resample_period'])
        .mean()
...
def process_candump_file(
    ...
    reader = (
        vaex.open(input_file)
        .to_pandas_df(chunk_size=chunksize)
    )
