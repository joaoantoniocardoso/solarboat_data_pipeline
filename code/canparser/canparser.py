...
def parse_payload(
    ...
    if verbose: print(payload, len(payload))
    ...
    if verbose: print(parsed_payload)
...
def process_message(
    ...
    if parsed_data_dict_list is None:
        if verbose: print('parsed_data_dict_list =', parsed_data_dict_list, "parsed =", parsed, parsed['payload'])
        return None
...
def apply_and_expand(df: pd.DataFrame, f: Callable[[pd.DataFrame], pd.DataFrame]) -> pd.DataFrame:
...
def process_candump_file(
    ...
    for c_index, chunk in enumerate(reader):
        ...
        if os.path.isfile(output_file):
            if verbose: print('\t -> already converted, skipping this chunk...')
            continue
