...
def process_candump_file(
    ...
    telemetry_file = dataset_info['telemetry_path'] + '/' + dataset_info['period'] + '/' + telemetry_filename
    output_filename = 'unified_monotonic_data_' + dataset_info['period'] + output_file_format
    ...
    df_telemetry = df_telemetry.asfreq(dataset_info['period']).tz_localize(pytz.timezone('America/Sao_Paulo')).reset_index()
    ...
    df_forecast.index = df_forecast.reset_index()['timestamp'].dt.tz_convert(pytz.timezone('America/Sao_Paulo'))
    index = pd.DatetimeIndex(
        df_telemetry['timestamp'].to_numpy(),
    ...
    input_lines = len(df_telemetry) + len(df_forecast)
    output_lines = len(df)
